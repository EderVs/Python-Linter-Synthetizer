...
def change_space_block(program_name, terminals, inputoutput):
    """
    """
    composition = transformation_generator([inputoutput], terminals)

    input_space_block = ""
    k = 0
    while k < len(inputoutput[0]):
        if inputoutput[0][k] == " " or inputoutput[0][k] == "\t":
            k += 1
            continue
        break

    input_space_block = inputoutput[0][:k]
    new_name = "new" + program_name
    new_file = open(new_name, 'w')
    file = open(program_name, 'r')
    for line in file:
        if line[0] != "" and (line[0] == " " or line[0] == "\t"):
            j = 0
            while j < len(line) and (line[j] == " " or line[j] == "\t"):
                j += len(input_space_block)
            content = line[j:]
            number_of_no_word = j // len(input_space_block)
            new_no_word = ""
            for i in range(number_of_no_word):
                new_no_word += eval_composition(composition, input_space_block)
            new_line = new_no_word + content
...
# of these examples.
inputoutputs_vars = [["ejemplo_1", "_ejemplo_1_"], 
                ["este_es_el_ejemplo_2", "_este_es_el_ejemplo_2_"]]
inputoutput_spaces = ["   ejemplo3", "    ejemplo3"]
...
terminals_space_block = [add_space_begin_line_if_no_word,
            add_tab_begin_line_if_no_word, 
            change_beggining_to_tab, 
            change_beggining_to_space]

program_name = "example.py"
change_variables(program_name, terminals, inputoutputs_vars)
change_space_block("_"+program_name, terminals_space_block, inputoutput_spaces)
